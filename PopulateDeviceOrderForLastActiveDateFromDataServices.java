...
public class PopulateDeviceOrderForLastActiveDateFromDataServices extends DataServicesDao {
	...
	protected void prevalidate() throws ApplicationException, EngineException {
		...
		billSystem =getInput(String.class,RoutingUtil.BILLING_SYS_CONTEXT_NAME);
	}
	...
	protected void populateEntities(Response resp) throws DataNotFoundException, ActionValidationException, SysDataNotFoundException, InternalResourceException, EngineException  {
		...
		for(int i=0;i<custDvcElementsList.size();i++){
			...
			if(billSysIdMaxValue == custDvcElementsList.get(i).getBillSysId() && dvcEqpTransTsValue.equals(dvcEqpTransDate)){
				...
				setOutput(customer); 
				if (billSysIdMaxValue == 1) {
					customerMap.put(BillingSystem.VISION_EAST.toString(), customer);
				} else if (billSysIdMaxValue == 2) {
					customerMap.put(BillingSystem.VISION_WEST.toString(), customer);
				} else if (billSysIdMaxValue == 7) {
					customerMap.put(BillingSystem.VISION_NORTH.toString(), customer);
				} else {
					customerMap.put(BillingSystem.VISION_B2B.toString(), customer);
				}
				//String dbBillSysId = String.valueOf(billSysIdMaxValue);
				//customerMap.put(dbBillSysId, customer);
	...
	private Customer createEmptyCustomer(String billingSystem) {
		Customer customer = new Customer(ZERO_STRING);
